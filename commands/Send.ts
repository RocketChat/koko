...
export const processSendCommand = async (
	...
	if (!(await hasValidRole(read, sender.roles, app.managerRolesMap))) {
		...
		app.getLogger().log(`Allowed roles: ${Array.from(app.managerRolesMap.values())}`);
		await notifyUser(
			app,
			modify,
			room,
	...
	if (!triggerId) {
		...
		await notifyUser(app, modify, room, sender, 'Unable to open message composer. Please try again.');
		return;
	}
	...
	try {
		...
		app.getLogger().error(`Error opening Send modal: ${errorMessage}`, error);
