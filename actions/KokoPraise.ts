...
export class KokoPraise {
	...
	public async run(
		...
		if (
			...
			this.app.kokoPostPraiseRoom !== undefined
		) {
			...
			if (sendScoreBoard === 'praisers') {
				await this.sendKarmaScoreboard({
					read,
					modify,
					room: this.app.kokoPostPraiseRoom,
					praisees: false,
					praisers: true,
				});
	 ...
	 * @param modify
	 */
	public async sendKarmaScoreboard({
		read,
		modify,
		room,
		user,
	...
	public async getUsernameFromText(read: IRead, text?: string): Promise<string | false> {
		...
		if (username) {
			...
			if (
				Array.from(members).some((member: IUser) => {
	...
	private async sendPraise(
		...
		let monthlyKarma = monhtlyKarmaData && monhtlyKarmaData.length > 0 && monhtlyKarmaData[0];
		if (!monthlyKarma) {
			monthlyKarma = {};
		}
		...
		const praiserKarmaData = await read.getPersistenceReader().readByAssociation(praiserKarmaAssoc);
		let praiserKarma =
			praiserKarmaData && praiserKarmaData.length > 0 && (praiserKarmaData[0] as IPraiserKarmaStorage);
		if (!praiserKarma) {
			praiserKarma = {};
		}
		const monthlyPraiserKarmaAssoc = new RocketChatAssociationRecord(
			RocketChatAssociationModel.MISC,
			'monthlyPraiserKarma',
		);
		const monthlyPraiserKarmaData = await read.getPersistenceReader().readByAssociation(monthlyPraiserKarmaAssoc);
		let monthlyPraiserKarma =
			monthlyPraiserKarmaData && monthlyPraiserKarmaData.length > 0 && monthlyPraiserKarmaData[0];
		...
		const senderUsername = Buffer.from(sender.username).toString('base64');
		...
		msg = msg
			...
			.replace('@username', '@' + replaceUsernames)
			.replace('{text}', text);
