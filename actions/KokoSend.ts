...
export class KokoSend {
	...
	public async submit({
		...
		// Find target room
		try {
			const targetRoom = await this.resolveTargetRoom(read, modify, roomName);
			...
			this.app.getLogger().error(`Error in send command: ${error.message}`);
	 ...
	 */
	private async resolveTargetRoom(read: IRead, modify: IModify, target: string): Promise<IRoom | undefined> {
		// Handle channel
