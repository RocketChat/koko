...
export class KokoQuestion {
    ...
    public async run(read: IRead, modify: IModify, persistence: IPersistence) {
        ...
        if (shouldUseOpenAI) {
            ...
            const response = await this.app.ai.getCompletionsForPrompt(prompt);
            if (response.data?.choices?.[0]?.messages?.content) {
                question = response.data?.choices?.[0]?.messages?.content;
            } else {
                this.app.getLogger().error('No question generated by OpenAI');
                return;
            }
        } else {
