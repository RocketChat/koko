...
export class KokoOneOnOne {
	...
	public async answer(
		...
		if (data.listen === 'one-on-one') {
			...
			if (text === 'Yes') {
				const monhtlyStatsAssoc = new RocketChatAssociationRecord(
					RocketChatAssociationModel.MISC,
					'one-on-one-monthly-stats',
				);
				const monthlyStatsData = await read.getPersistenceReader().readByAssociation(monhtlyStatsAssoc);
				let monthlyStats = monthlyStatsData && monthlyStatsData.length > 0 && monthlyStatsData[0];
				if (!monthlyStats) {
					monthlyStats = {};
				}
				...
				// Checks if user is first or last in the call
				const oneOnOneAssociation = new RocketChatAssociationRecord(
					RocketChatAssociationModel.MISC,
					'one-on-one',
				);
				...
				if (oneOnOneData && oneOnOneData.length > 0 && oneOnOneData[0]) {
					...
					if (username !== sender.username) {
						...
						const statsAssoc = new RocketChatAssociationRecord(
							RocketChatAssociationModel.MISC,
							'one-on-one-stats',
						);
						const stats: IStatsStorage = {
							username1: username,
