...
export class KokoValues {
		...
		if (this.app.botUser !== undefined && this.app.kokoMembersRoom !== undefined) {
			...
			for (const member of members) {
				...
				await sendMessage(
					...
					modify,
					room,
					'Rocket.Chat values are easily remembered by looking at our logo. Do you know what the DOTS mean?',
					[],
					blocks,
				);
			}
	 ...
	 */
	private async sendValuesAnswer(
		read: IRead,
		modify: IModify,
		persistence: IPersistence,
		sender: IUser,
		...
		if (usernames === undefined || usernames.length === 0) {
			...
			msg =
				'Here is something @sender thinks @username did, that is connected to our value(s) of *{values}*:\n{text}';
		} else {
			msg =
				'Here is something @sender thinks @username did, that is connected to our value(s) of *{values}*:\n{text}';
